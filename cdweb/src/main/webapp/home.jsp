					...
					<c:choose>
						<c:when test="${count.index eq 0}">
							<div class="carousel-item active">
								<img src="${movie.largeImageURL}" alt="Los Angeles" width="1100"
									height="500">
							</div>
						</c:when>
						<c:otherwise>
		...
		<div class="d-flex justify-content-between flex-wrap">
			<c:forEach items="${movies}" var="movie">
				<div class="card movie-item" style="width: 300px">
					<img class="card-img-top img-movie" src="${movie.smallImageURl}"
						alt="Card image" style="width: 100%">
					<div class="card-body">
						<h4 class="card-title">${movie.name}</h4>
						<p class="card-text">${movie.shortDescription}</p>
						<a href="/movie-details?movieId=${movie.id}"
							class="btn btn-outline-warning" style="margin-right: 70px">Chi
							tiết</a>
						...
						<c:choose>
							<c:when test="${sessionScope.jwtResponse eq null}">
								<button
									class="btn btn-outline-danger btn-buy-ticket-not-signed-in">Mua
									vé</button>
							</c:when>
							<c:otherwise>
								<a href="/branches?movieId=${movie.id}"
									class="btn btn-outline-danger">Mua vé</a>
							</c:otherwise>
						</c:choose>
					</div>
				</div>
