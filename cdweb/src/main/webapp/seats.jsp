...
<body>
	...
	<div class="screen-container">
		...
		<br>
		<br>
		<p style="color: red">${bookedError}</p>
		<div class="container">
			<form action="bill" method="post">
						...
						<c:forEach items="${listA}" var="seat">
							<c:choose>
								<c:when test="${seat.isOccupied eq 1}">
									<th><input type="checkbox" class="largerCheckbox"
										name="seats" value="${seat.id}" checked disabled></th>
								</c:when>
								<c:otherwise>
									<th><input type="checkbox" class="largerCheckbox"
										name="seats" value="${seat.id}"></th>
								</c:otherwise>
							</c:choose>
				...
				<br> <br> <input type="submit"
					class="btn btn-outline-danger btn-block" value="Tiếp Tục"></input>
			</form>
		</div>
	</div>
